...
class Image
{
    ...
    public function __construct($filePath = false)
    {
        ...
        // Setting up Intervention to use GD lib
        Intervention::configure(array('driver' => 'gd'));
    }
    ...
    protected function watermarkImage()
    {
        ...
        $image = Intervention::make($filePath);
        $image->insert(base_path() . config('cms.storage.media.path') . $this->settings->watermark_image_path, 'bottom-right');
        $image->save($filePath);
    }
    ...
    /**
     * Checks if watermarking is enabled for this image
     * @return bool
     */
    protected function isWatermarkEnabled()
    {
        return ($this->settings->enable_watermark && $this->options['watermark']);
    }
